...
- **Cost levers:** `HISTORY_TURNS` (capped at 2), `MAX_RESULTS` (retrieved chunks), `MODEL`, `MAX_OUTPUT_TOKENS`. **Prompt caching is intentionally not used** — the system prompt is below Haiku's 4,096-token cache floor and the retrieved chunks differ every call, so there's nothing cacheable.
